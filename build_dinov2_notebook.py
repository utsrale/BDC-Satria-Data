...
notebook_path = 'd:/Pancar/KULIAH UNS/LOMBA/Satria Data 2026/BDC SatDat 2026/train_dinov2.ipynb'
...
cells.append({
    ...
    "source": [
        ...
        "## Notebook Klasifikasi Limbah - DINOv2 Large Linear Probing\n",
        "\n",
        "**Strategi:**\n",
        "1. **Giant Feature Extractor:** Menggunakan DINOv2 Large (`vit_large_patch14_dinov2.lvd142m`) dengan 300 juta parameter.\n",
        "2. **Full Backbone Freeze:** Mengunci seluruh parameter backbone Meta DINOv2 agar fitur tidak rusak/overfit.\n",
        "3. **Advanced MLP Head Tuning:** Menyetel kepala klasifikasi multi-layer (Linear -> BatchNorm -> ReLU -> Dropout -> Linear) dengan LR `1e-3`.\n",
        "4. **Super Fast Training:** Training per fold sangat cepat karena hanya melatih parameter classifier head.\n",
...
# Sel 6: Augmentasi DINOv2 (224x224 native resolution)
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 6. AUGMENTASI DATA (DINOv2 Native Resolution 518x518, Batch Size 32)\n",
        "IMG_SIZE = 518\n",
        "BATCH_SIZE = 32\n",
        "\n",
...
# Sel 7: Custom Model Class DINOv2 Large
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    ...
    "source": [
        ...
        "            \n",
        "        num_features = self.backbone.num_features # Untuk ViT-Large = 1024\n",
        "        \n",
...
# Sel 9: Helper K-Fold untuk DINOv2
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    ...
    "source": [
        ...
        "def train_model_kfold(model_name='vit_large_patch14_dinov2.lvd142m', n_splits=5, num_epochs=10):\n",
        "    oof_predictions = np.zeros(len(df_train), dtype=int)\n",
        "    oof_true_labels = np.zeros(len(df_train), dtype=int)\n",
        "    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=SEED)\n",
        "    fold_results = []\n",
        "    \n",
        "    log_file_path = PROJECT_ROOT / f'training_log_dinov2.txt'\n",
        "    with open(log_file_path, 'w') as f:\n",
        "        f.write(f'=== LOG TRAINING DINOv2 LARGE ===\\n\\n')\n",
        "        \n",
        ...
        "            f.write(message + '\\n')\n",
        "            \n",
        "    write_log(f'Memulai training DINOv2 Large ({n_splits}-Fold)...\\n')\n",
        "    \n",
        "    for fold, (train_idx, val_idx) in enumerate(skf.split(df_train, df_train['label'])):\n",
        "        try:\n",
        "            write_log('=' * 50)\n",
        "            write_log(f' DINOv2 LARGE | FOLD {fold + 1} / {n_splits}')\n",
        "            write_log('=' * 50)\n",
        "            \n",
        ...
        "            \n",
        "            # Buat model\n",
        "            model = Dinov2WasteClassifier(model_name=model_name, num_classes=len(CLASS_NAMES))\n",
        ...
        "            \n",
        "            best_val_f1 = 0.0\n",
        "            best_model_path = MODELS_DIR / f'best_dinov2_fold_{fold}.pth'\n",
        "            \n",
        ...
        "        \n",
        "    write_log('=' * 50)\n",
        "    write_log(f' TRAINING DINOv2 LARGE K-FOLD SELESAI')\n",
        "    write_log('=' * 50)\n",
...
cells.append({
    ...
    "outputs": [],
    "source": [
        "# 10. EKSEKUSI TRAINING DINOv2 LARGE\n",
        "N_SPLITS = 5\n",
        "EPOCHS = 10\n",
        "\n",
        "dinov2_results = train_model_kfold(model_name='vit_large_patch14_dinov2.lvd142m', n_splits=N_SPLITS, num_epochs=EPOCHS)"
    ]
})

# Sel 11: Ensemble Inference & Export Submission
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
...
print("Notebook DINOv2 Large sukses dibangun!")
