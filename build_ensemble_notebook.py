...
cells.append({
    ...
    "source": [
        ...
        "        return out\n",
        "\n",
        "class ClipWasteClassifier(nn.Module):\n",
        "    def __init__(self, model_name='vit_large_patch14_clip_224', num_classes=3):\n",
        "        super().__init__()\n",
        "        self.backbone = timm.create_model(model_name, pretrained=False, num_classes=0)\n",
        "        num_features = self.backbone.num_features\n",
        "        self.head = nn.Sequential(\n",
...
cells.append({
    ...
    "source": [
        ...
        "clip_weights_paths = find_model_weights(\"best_clip_fold_*.pth\")\n",
        "\n",
        ...
        "    print(f\"  - {p}\")\n",
        "\n",
        "print(\"\\nBerkas bobot CLIP ditemukan:\")\n",
        "for p in clip_weights_paths:\n",
        "    print(f\"  - {p}\")"
    ]
})
...
cells.append({
    ...
    "source": [
        ...
        "    model_instance.eval()\n",
        "    models.append((model_instance, 0.45)) # Bobot DINOv2 = 0.45\n",
        "    print(f\"Loaded DINOv2 model: {os.path.basename(p)} (weight: 0.45)\")\n",
        "\n",
        "# Load CLIP Models\n",
        "for p in clip_weights_paths:\n",
        "    model_instance = ClipWasteClassifier(model_name='vit_large_patch14_clip_224', num_classes=len(CLASS_NAMES))\n",
        "    model_instance.load_state_dict(torch.load(p, map_location=device))\n",
        "    model_instance = model_instance.to(device)\n",
        "    model_instance.eval()\n",
        "    models.append((model_instance, 0.45)) # Bobot CLIP = 0.45\n",
        "    print(f\"Loaded CLIP model: {os.path.basename(p)} (weight: 0.45)\")\n",
        "\n",
        "# Load RegNetY Models\n",
        "for p in regnet_weights_paths:\n",
        ...
        "    model_instance = model_instance.to(device)\n",
        "    model_instance.eval()\n",
        "    models.append((model_instance, 0.10)) # Bobot RegNetY = 0.10\n",
        "    print(f\"Loaded RegNetY model: {os.path.basename(p)} (weight: 0.10)\")\n",
        "\n",
