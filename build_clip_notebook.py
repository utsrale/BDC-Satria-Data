...
notebook_path = 'd:/Pancar/KULIAH UNS/LOMBA/Satria Data 2026/BDC SatDat 2026/train_clip.ipynb'
...
cells.append({
    ...
    "source": [
        ...
        "## Notebook Klasifikasi Limbah - CLIP Large VLM Feature Extraction\n",
        "\n",
        "**Strategi:**\n",
        "1. **Vision-Language Model (VLM):** Menggunakan encoder visual CLIP Large (`vit_large_patch14_clip_224`) dengan 300 juta parameter.\n",
        "2. **Full Backbone Freeze:** Mengunci seluruh parameter backbone CLIP OpenAI agar representasi semantik bahasa-gambar tidak rusak/overfit.\n",
        "3. **Advanced MLP Head Tuning:** Menyetel kepala klasifikasi multi-layer (Linear -> BatchNorm -> ReLU -> Dropout -> Linear) dengan LR `1e-3`.\n",
        "4. **5-Fold Cross Validation & 10 Epoch:** Latihan stabil dengan ensemble 5 model terbaik."
    ]
})
...
# Sel 6: Augmentasi CLIP (224x224 native resolution)
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 6. AUGMENTASI DATA (CLIP Native Resolution 224x224, Batch Size 64)\n",
        "IMG_SIZE = 224\n",
        "BATCH_SIZE = 64\n",
        "\n",
...
# Sel 7: Custom Model Class CLIP Large
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 7. MODEL DEFINITION: CLIP Large with MLP Classifier Head\n",
        "class ClipWasteClassifier(nn.Module):\n",
        "    def __init__(self, model_name='vit_large_patch14_clip_224', num_classes=3):\n",
        "        super().__init__()\n",
        "        # Load pre-trained backbone\n",
        "        print(f\"Membuat model backbone CLIP Large: {model_name}...\")\n",
        "        self.backbone = timm.create_model(model_name, pretrained=True, num_classes=0)\n",
        "        \n",
        "        # Kunci (freeze) seluruh parameter backbone\n",
        "        for param in self.backbone.parameters():\n",
        "            param.requires_grad = False\n",
        "            \n",
        "        num_features = self.backbone.num_features # Untuk vit_large_patch14_clip_224 = 768\n",
        "        \n",
        "        # Classifier Head (2-Layer MLP dengan BatchNorm, ReLU, Dropout)\n",
...
# Sel 8: Training & Validation Helper
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 8. FUNGSI TRAINING ONE EPOCH & VALIDATE (Log Bersih Per Epoch)\n",
...
# Sel 9: Helper K-Fold untuk CLIP
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 9. HELPER TRAINING K-FOLD (5-Fold, 10 Epoch)\n",
        "def train_model_kfold(model_name='vit_large_patch14_clip_224', n_splits=5, num_epochs=10):\n",
        "    oof_predictions = np.zeros(len(df_train), dtype=int)\n",
        "    oof_true_labels = np.zeros(len(df_train), dtype=int)\n",
        "    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=SEED)\n",
        "    fold_results = []\n",
        "    \n",
        "    log_file_path = PROJECT_ROOT / f'training_log_clip.txt'\n",
        "    with open(log_file_path, 'w') as f:\n",
        "        f.write(f'=== LOG TRAINING CLIP LARGE ===\\n\\n')\n",
        "        \n",
        ...
        "            f.write(message + '\\n')\n",
        "            \n",
        "    write_log(f'Memulai training CLIP Large ({n_splits}-Fold)...\\n')\n",
        "    \n",
        "    for fold, (train_idx, val_idx) in enumerate(skf.split(df_train, df_train['label'])):\n",
        "        try:\n",
        "            write_log('=' * 50)\n",
        "            write_log(f' CLIP LARGE | FOLD {fold + 1} / {n_splits}')\n",
        "            write_log('=' * 50)\n",
        "            \n",
        ...
        "            \n",
        "            # Buat model\n",
        "            model = ClipWasteClassifier(model_name=model_name, num_classes=len(CLASS_NAMES))\n",
        "            model = model.to(device)\n",
        "            scaler = torch.amp.GradScaler('cuda') if device.type == 'cuda' else None\n",
        "            \n",
        "            # Bobot loss untuk class imbalance\n",
        "            class_weights = torch.tensor([1.25, 3.17, 1.00]).to(device)\n",
        "            criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.1)\n",
        "            \n",
        "            # Hanya tune Classifier Head (AdamW, LR=1e-3, weight_decay=1e-2)\n",
        "            optimizer = optim.AdamW(model.head.parameters(), lr=1e-3, weight_decay=1e-2)\n",
        "            scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs)\n",
        "            \n",
        "            best_val_f1 = 0.0\n",
        "            best_model_path = MODELS_DIR / f'best_clip_fold_{fold}.pth'\n",
        "            \n",
        ...
        "            \n",
        "            # Simpan hasil OOF untuk fold ini\n",
        "            for idx, pred_val, true_val in zip(val_idx, best_val_preds, best_val_labels):\n",
        "                oof_predictions[idx] = pred_val\n",
        "                oof_true_labels[idx] = true_val\n",
        "            \n",
        ...
        "        \n",
        "    write_log('=' * 50)\n",
        "    write_log(f' TRAINING CLIP LARGE K-FOLD SELESAI')\n",
        "    write_log('=' * 50)\n",
        "    write_log(f'Rata-rata Macro F1-Score: {np.mean(fold_results):.4f} (+/- {np.std(fold_results):.4f})\\n')\n",
        "    \n",
        "    # Laporan Evaluasi Akhir secara Detail (Classification Report)\n",
        "    write_log('=' * 50)\n",
        "    write_log(' LAPORAN EVALUASI OUT-OF-FOLD (OOF) - DETAIL')\n",
        "    write_log('=' * 50)\n",
        "    write_log(classification_report(oof_true_labels, oof_predictions, target_names=CLASS_NAMES))\n",
        "    write_log('=' * 50)\n",
        "    return fold_results"
    ]
})

# Sel 10: Eksekusi Training
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 10. EKSEKUSI TRAINING CLIP LARGE\n",
        "N_SPLITS = 5\n",
        "EPOCHS = 10\n",
        "\n",
        "clip_results = train_model_kfold(model_name='vit_large_patch14_clip_224', n_splits=N_SPLITS, num_epochs=EPOCHS)"
    ]
})

# Sel 11: Ekspor Submission Sementara (Hanya CLIP)
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    ...
    "source": [
        ...
        "output_file = PROJECT_ROOT / 'submission_clip_only.csv'\n",
        "df_submission.to_csv(output_file, index=False)\n",
        "\n",
        "print(f'\\nCLIP Submission sukses disimpan di: {output_file}')\n",
...
print("Notebook CLIP Large sukses dibangun!")
