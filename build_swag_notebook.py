...
notebook_path = 'd:/Pancar/KULIAH UNS/LOMBA/Satria Data 2026/BDC SatDat 2026/train_swag.ipynb'
...
cells.append({
    ...
    "source": [
        ...
        "## Notebook Klasifikasi Limbah - RegNetY SWAG Feature Extraction\n",
        "\n",
        "**Strategi:**\n",
        "1. **Weakly-Supervised CNN:** Menggunakan model RegNetY SWAG (`regnety_160_swag_in1k`) yang dilatih pada miliaran gambar.\n",
        "2. **Full Backbone Freeze:** Mengunci seluruh parameter backbone RegNetY agar training berjalan super cepat dan terhindar dari overfitting.\n",
        "3. **Advanced MLP Head Tuning:** Menyetel kepala klasifikasi multi-layer (Linear -> BatchNorm -> ReLU -> Dropout -> Linear) dengan LR `1e-3`.\n",
        "4. **Complementary Ensemble Part:** Model CNN ini akan digabungkan dengan DINOv2 Large (Transformer) via soft voting untuk akurasi maksimal.\n",
        "5. **5-Fold Cross Validation & 10 Epoch:** Latihan stabil dengan ensemble 5 model terbaik."
    ]
})
...
# Sel 6: Augmentasi RegNetY (224x224, Batch Size 128 karena CNN sangat efisien)
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 6. AUGMENTASI DATA (Batch Size 128 untuk training cepat)\n",
        "IMG_SIZE = 224\n",
        "BATCH_SIZE = 128\n",
        "\n",
...
# Sel 7: Custom Model Class RegNetY SWAG
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    "outputs": [],
    "source": [
        "# 7. MODEL DEFINITION: RegNetY SWAG with MLP Classifier Head\n",
        "class RegNetYSwagClassifier(nn.Module):\n",
        "    def __init__(self, model_name='regnety_160_swag_in1k', num_classes=3):\n",
        "        super().__init__()\n",
        "        # Load pre-trained backbone\n",
        "        print(f\"Membuat model backbone RegNetY SWAG: {model_name}...\")\n",
        ...
        "            \n",
        "        num_features = self.backbone.num_features # Untuk regnety_160 = 3024\n",
        "        \n",
...
# Sel 9: Helper K-Fold untuk RegNetY SWAG
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    ...
    "source": [
        ...
        "def train_model_kfold(model_name='regnety_160_swag_in1k', n_splits=5, num_epochs=10):\n",
        "    oof_predictions = np.zeros(len(df_train), dtype=int)\n",
        "    oof_true_labels = np.zeros(len(df_train), dtype=int)\n",
        "    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=SEED)\n",
        "    fold_results = []\n",
        "    \n",
        "    log_file_path = PROJECT_ROOT / f'training_log_regnet.txt'\n",
        "    with open(log_file_path, 'w') as f:\n",
        "        f.write(f'=== LOG TRAINING REGNETY SWAG ===\\n\\n')\n",
        "        \n",
        ...
        "            f.write(message + '\\n')\n",
        "            \n",
        "    write_log(f'Memulai training RegNetY SWAG ({n_splits}-Fold)...\\n')\n",
        "    \n",
        "    for fold, (train_idx, val_idx) in enumerate(skf.split(df_train, df_train['label'])):\n",
        "        try:\n",
        "            write_log('=' * 50)\n",
        "            write_log(f' REGNETY SWAG | FOLD {fold + 1} / {n_splits}')\n",
        "            write_log('=' * 50)\n",
        "            \n",
        ...
        "            \n",
        "            # Buat model\n",
        "            model = RegNetYSwagClassifier(model_name=model_name, num_classes=len(CLASS_NAMES))\n",
        ...
        "            \n",
        "            best_val_f1 = 0.0\n",
        "            best_model_path = MODELS_DIR / f'best_regnet_fold_{fold}.pth'\n",
        "            \n",
        ...
        "        \n",
        "    write_log('=' * 50)\n",
        "    write_log(f' TRAINING REGNETY SWAG K-FOLD SELESAI')\n",
        "    write_log('=' * 50)\n",
...
cells.append({
    ...
    "outputs": [],
    "source": [
        "# 10. EKSEKUSI TRAINING REGNETY SWAG\n",
        "N_SPLITS = 5\n",
        "EPOCHS = 10\n",
        "\n",
        "regnet_results = train_model_kfold(model_name='regnety_160_swag_in1k', n_splits=N_SPLITS, num_epochs=EPOCHS)"
    ]
})

# Sel 11: Ekspor Submission Sementara (Hanya RegNetY)
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {},
    ...
    "source": [
        ...
        "output_file = PROJECT_ROOT / 'submission_regnet_only.csv'\n",
        "df_submission.to_csv(output_file, index=False)\n",
        "\n",
        "print(f'\\nRegNetY Submission sukses disimpan di: {output_file}')\n",
...
print("Notebook RegNetY SWAG sukses dibangun!")
